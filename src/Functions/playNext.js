...
const playNext=async(image,title,id,channelName,songs,currentPlaying,name, artistId="")=>{
    if(songs){
        const index = songs.findIndex(data => data.id === currentPlaying.id)
        songs.splice(index+1,0,{image,title,id,channelName,playedBy:name, artistId})
