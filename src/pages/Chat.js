    ...
    const updateRead = () => {
        try{
            const unReadMsg = messages.filter(
              (data) => (data.status = "unread" && data.name !== Cookies.get("name"))
            );
            if(unReadMsg.length > 0 && messages.length > 0){
                unReadMsg.map((data) =>
                  updateDoc(
                    doc(
                      db,
      ...
      <div className="flex  flex-col w-screen">
        {sessionStorage.getItem("roomCode") ? (
          <div>
              ...
              <div className="flex flex-col justify-between w-[95vw] gap-0">
                {messages.map((data, index) =>
                  data.sender === Cookies.get("name") ? (
                    ...
                    <div
                      key={index}
                      className=" chat chat-start  text-white p-2 "
                    >
                      <div className="bg-black chat-bubble">
                        <b>{data.sender}</b>
                        <p>{data.data}</p>
                ...
                <button
                  className={` bg-white bg-opacity-10 w-8 p-2  rounded-full backdrop-filter backdrop-blur fixed right-7 bottom-20 transition-opacity ${
            ...
            <div className=" absolute ms-3 bottom-32">
              <div className="flex flex-row gap-2">
                ...
                />
                <button
                  className="bg-slate-50 rounded-full  p-2 w-10  text-center "
                  onClick={() => sendMsg()}
                >
          ...
          <div className="mt-14 mx-auto text-white">
            <p>
              <b>Please join the room to chat </b>
            </p>
          </div>
        )}
      </div>
